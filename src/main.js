...
const sidebarClickedTaskName = document.getElementById(
  "sidebarClickedTaskName"
); // sidebar clicked li item name
  ...
  if (target) {
    ...
    if (event.type === "click") {
      document.querySelectorAll("[sidebar-item]").forEach(
        (el) => el.classList.remove("active", ...activeClasses) //* Remove 'active' class from all sidebar items
      );
...
const todos = [
  ...
  new Todo({
    ...
    task_list_type: "Work",
    task_date: "2024-05-16",
    // task_time: new Date(["2025-05-17T17:01"]),
    subtask_no: 1,
  }),
  new Todo({
    ...
    task_list_type: "",
    task_date: "2027-05-16",
    // task_time: new Date(["2025-05-07T03:06"]),
    subtask_no: 2,
    is_important: true,
    is_done: true,
  }),
  new Todo({
    ...
    task_date: "2025-05-17",
    // task_time: new Date(["2025-05-15T17:13"]),
    subtask_no: 3,
  }),
  new Todo({
    ...
    task_description: "Consult accountent",
    task_list_type: "Other",
    task_date: "2025-05-18",
    // task_time: new Date(["2025-12-27T17:13"]),
...
addNewTaskBtn.addEventListener("click", () => {
  ...
  taskListContainer.classList.add("w-[calc(100%-320px)]");

  addTaskForm.classList.remove("hidden", "w-0");
  addTaskForm.classList.add("w-[320px]");
});
function hideFormContainer() {
  taskListContainer.classList.remove("w-[calc(100%-320px)]");
  taskListContainer.classList.add("w-full");

  addTaskForm.classList.add("hidden");
  addTaskForm.classList.remove("w-[320px]");
...
function renderTodolist() {
  ...
  const filtered_todos = todos.filter((todo) => {
    ...
    const givenDate = todo.task_date;
    ...
    if (filter == "all") {
      ...
      const currentDate = currentDate.toDateString();
      const givenDate = givenDate.toDateString();
    ...
    todo_list += `<li data-id=${elem.id}>
            <div class="flex justify-between items-center bg-white px-3 py-2 gap-8 rounded-lg shadow-sm border border-gray-300">
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="todo-checkbox-${index}" class="todo-checkbox flex-none mr-2 text-start" ${
      elem.is_done ? "checked" : ""
    }>
                        ...
                        ${
                          elem.task_name
                            ? `<label for="todo-checkbox-${index}" class="${
                                elem.is_done ? "line-through text-gray-600" : ""
                              } grow select-none">${elem.task_name}</label>`
                            : ""
                        }          
...
addTaskForm.addEventListener("submit", function (event) {
  ...
  const form_input_date = dueDate.value;
  // const due_date = formInputDate ? new Date(formInputDate) : null;

  const form_input_time = dueTime.value;

  // console.log(due_date);
  // if (formInputTime) {
  //     const current_date = getDate(new Date());
  //     formInputDate = formInputDate || current_date;
  // }

  const due_date = form_input_date ? form_input_date : null;

  // if (filter === "upcoming" && isNaN(due_date)) {
  ...
  if (todo_list_id) {
    ...
    if (todo) {
      ...
      //  if (due_date) {
      todo.task_date = form_input_date;
      // }

      // todo.task_time = task_time;
    }
  } else {
...
todoList.addEventListener("click", function (event) {
  ...
  //& mark important
  const markBtn = event.target.closest(
    `button.todo-important-btn[type="button"]`
  );
  if (markBtn) {
